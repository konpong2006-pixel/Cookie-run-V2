...
class Vision:
    ...
    def extract_obstacle_features(self, img):
        ...
        # 2. Level 2: Edge features. Prefer HOG, but some OpenCV builds do not include it.
        ...
        if hasattr(cv2, "HOGDescriptor"):
            try:
                ...
                hog_features = hog.compute(hog_img).flatten().astype(np.float32)
            except Exception as e:
                print(f"[Vision] HOG unavailable, using edge fallback: {e}")

        if hog_features is None:
            edges = cv2.Canny(hog_img, 80, 160)
            hog_features = cv2.resize(edges, (27, 28)).flatten().astype(np.float32)
    ...
    def is_relay_window(self, img, rect_pct):
        ...
        for y in range(y1, y2, 3): # Check every 3rd pixel for better reliability
            for x in range(x1, x2, 3):
                r, g, b = img.getpixel((x, y))
                # Relay buttons are green, but brightness shifts by emulator and stage effects.
                if g > 120 and g > r + 20 and g > b + 20:
                    green_count += 1
                total_pixels += 1
                
        return total_pixels > 0 and green_count >= 20 and (green_count / total_pixels) > 0.05
